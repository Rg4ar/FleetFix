...
public class Conexion {
    ...
    private static void inicializarDB(Connection con) throws SQLException {
        ...
        if (rs.next() && rs.getInt(1) == 0) {
            st.execute("INSERT INTO Vehiculos (placa, modelo, estado) VALUES ('ABC-001', 'Toyota Hilux', 'Operativo')");
            st.execute("INSERT INTO Vehiculos (placa, modelo, estado) VALUES ('XYZ-002', 'Ford F-150', 'Revision')");
            st.execute("INSERT INTO Vehiculos (placa, modelo, estado) VALUES ('DEF-003', 'Nissan Frontier', 'Operativo')");
            st.execute("INSERT INTO Vehiculos (placa, modelo, estado) VALUES ('GHI-004', 'Chevrolet Silverado', 'Operativo')");
        }
        st.close();
    }
}